    ...
    stages{
       ...
       stage('Build Stage'){
        ...
        post {
                success {
                    script {
                        def server = Artifactory.newServer(url: 'http://3.110.208.138:8081/artifactory/', credentialsId: 'jfrog')
                        def rtMaven = Artifactory.newMavenBuild()
                        //rtMaven.deployer server: server, releaseRepo: 'libs-release/', snapshotRepo: 'libs-snapshot/'
                        rtMaven.deployer server: server, releaseRepo: 'psm/', snapshotRepo: 'psm/'
                        rtMaven.tool = 'maven'
